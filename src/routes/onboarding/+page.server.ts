...
export const load: PageServerLoad = async (event) => {
	...
	if (userId) {
		userOrganization = await supabase
			.from('organizations')
			.select(
				'organization_name, public_email, public_phone, public_address, show_email, show_phone, show_address'
			)
			.eq('owner_id', userId)
			.single();
		...
		if (userOrganization.error?.details) {
			if (userOrganization.error.details.includes('Results contain 0 rows'))
